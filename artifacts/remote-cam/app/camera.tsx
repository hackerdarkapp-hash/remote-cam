...
export default function CameraScreen() {
  ...
  // Load room id + auto-start preference
  useEffect(() => {
  ...
  // JS Socket.IO (used on Expo Go / iOS / web; on Android APK the native service handles its own socket)
  useEffect(() => {
    ...
    socket.on("connect", () => { setConnected(true); socket.emit("camera-join", { roomId }); });
  ...
  // App state — only matters for JS streaming path
  useEffect(() => {
  ...
  // JS capture loop (Expo Go / iOS / web only)
  ...
  return (
    <View style={[styles.container, { backgroundColor: "#000" }]}>
      ...
      {/* Camera preview — shown always (native service handles capture in background on APK) */}
      <CameraView ref={cameraRef} style={StyleSheet.absoluteFill} facing="back" />

      {!isStreaming && <View style={[StyleSheet.absoluteFill, styles.overlay]} />}

      {/* Background mode overlay (JS streaming path only) */}
      ...
      {/* Auto-start toggle — only visible on Android APK */}
      {isNativeStreamingAvailable && (
